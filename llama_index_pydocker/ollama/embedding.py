"""Ollama embedding model with automatic Docker provisioning."""
...
class OllamaEmbedding(_OllamaEmbedding):
    """
    ...
    ----------
    model_name : str
        Ollama embedding model name, e.g. ``"nomic-embed-text"`` or
        ``"mxbai-embed-large"``.
    base_url : str
    ...
    **kwargs
        Forwarded to the upstream ``OllamaEmbedding.__init__``
        (e.g. ``embed_batch_size``, ``ollama_additional_kwargs``).
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Initialize an Ollama embedding model with optional Docker provisioning.
        ...
        **kwargs
            Extra keyword arguments forwarded to the base embedding model.
        """
        ...
        if is_localhost(host):
            ...
            _db = OllamaDB(cfg)
            _db.create_db()
            _ensure_model(_db, base_url, model_name)

        super().__init__(model_name=model_name, base_url=base_url, **kwargs)
        # Bypass Pydantic's __setattr__ — OllamaEmbedding extends BaseModel.
    ...
    def __enter__(self):
        ...
        -------
        OllamaEmbedding
            The current embedding instance.
        """
        return self
...
def _ensure_model(db: OllamaDB, base_url: str, model_name: str) -> None:
    """Ensure an Ollama embedding model is available locally.
    ...
    if model_name not in present and f"{model_name}:latest" not in present:
        pull_model_verbose(base_url, model_name)
